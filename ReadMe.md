...
또한 SignalR과 CameraOrchestrator를 연계하여  
...
본 시스템은 \*\*생산 → 재고 → 출고 흐름\*\*을 기준으로 설계되었습니다.
...
1\. 카메라(RTSP)를 통해 생산 이벤트 감지 → `ProductionEvent` 저장  

2\. 생산 완료된 제품은 `Inventory`에 반영  

3\. 납품 요청은 `DeliveryItem`으로 관리  

4\. 최종 출고 시 `StockOutHistory` 기록  
...
\- 실시간 제어 및 상태 반영 (SignalR)

\- 영상 분석 + 재고/출고 시스템 통합

\- 확장 가능한 구조 (멀티 카메라 대응 가능)


>>>>>>> f478a35 (Add realtime camera control with SignalR)
